...
from AVA_vit import AVADataset, get_train_val
...
def run(train_batch_size, val_batch_size, epochs, momentum, log_interval):
    ...
    #Load the vision transformer using the timm-library
    base_model = timm.create_model('vit_base_patch32_224_in21k', pretrained=True)
    config = resolve_data_config({}, model=base_model)
    preprocess = create_transform(**config)

    #Removing the classification head so we achieve the same output shape as with the CLIP-model
    ...
    writer.add_text('ViT_VARIANT', str(ViT_VARIANT))
